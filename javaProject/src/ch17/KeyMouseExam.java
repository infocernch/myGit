...
public class KeyMouseExam extends JApplet 
	...
	private int width, height;//이미지의 가로세로 사이즈
	...
	public void init() {
		...
		this.addMouseListener(this);//현재클래스에 마우스 이벤트기능 추가(중요코드)
		this.addMouseMotionListener(this);
		this.addKeyListener(this);//현재 클래스에 키 이벤트기능 추가
	...
	public void keyPressed(KeyEvent e) {
		...
		//max(값,값) 두 값중 더 큰값 구하기. min(값,값) 최솟값을 리턴받아
		//이미지가 프레임 바깥으로 빠져나가지않게 처리함..
	...
	//컴퍼넌트상에서 마우스버튼을 클릭(눌렀다 놓을때)했을때 호출
	@Override
	public void mouseClicked(MouseEvent e) {
		flag=true;//클릭할때 그림 true로 설정해서 보여지게함.
		x = e.getX();
		y = e.getY();
		repaint();//그래픽 갱신요청 => paint()가 호출
	}

	@Override
	public void mousePressed(MouseEvent e) {
		//컴퍼넌트상에서 마우스버튼을 누르는 시점 때에 호출
