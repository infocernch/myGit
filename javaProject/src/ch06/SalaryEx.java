...
public class SalaryEx {
	...
	static int[] salay = {2800,3000,2500,2000,3500};//연봉
	static int[] bonus = {0,0,0,0,0};//보너스
	static int[] total = new int[5];//총액(연봉+보너스)
	static int[] tax = new int[5];//세금처
	static int[] pay = new int[5];//실수령액
	static int pay_tot;//총지급액
	
	static void calc() {//계산
		for(int i=0;i<name.length;i++) {
			bonus[i] = (int)(salay[i] * 0.3);//연봉의 30%
			total[i] = salay[i] + bonus[i]; //총액(연봉+보너스
	...
	static void print() {//출력용도
		...
		for(int i=0;i<name.length;i++) {
			System.out.println(name[i]+"\t"+salay[i]+"\t"+bonus[i]+"\t"
					+total[i]+"\t"+tax[i]+"\t"+pay[i]);
		}
