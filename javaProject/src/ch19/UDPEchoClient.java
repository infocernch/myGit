...
public class UDPEchoClient {
	...
	public UDPEchoClient(String ip, int port) {
		try {
			...
			System.out.println("서버측에서 보내는 메시지 : ");
			...
			ds.receive(dp);//서버에서 보낸 매세지를 수신
			ds.close();//소켓 연결 닫기
			
			System.out.println("sever ip : "+dp.getAddress() + 
					", sever port:"+dp.getPort());
	...
	public static void main(String[] args) {
		...
		//UDP는 클라이언트 포트번호와 서버쪽 포트번호가 달라야한다.
