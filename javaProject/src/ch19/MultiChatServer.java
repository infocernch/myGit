...
public class MultiChatServer  {
	...
	public void serviceStart() {
		...
		try {
			...
			while (true) {
				...
				//클라이언트별로 메시지 수신을 위해 스레드가 필요하다
				//ex) 10명이 접속하면 10개의 스레드가 생성(속도저하)
	...
	void sendToAll(String msg) {
		//메시지를 받아서 일괄전송(HashMap에서 Key값들의 집합을 리턴받음)
	...
	class ServerReceiver extends Thread{
		...
		public ServerReceiver(Socket socket) {
			...
			try {
				...
				System.out.println("ServerReceiver 소켓 IO에러");
			}
		}//생성자
		@Override
		public void run() {
			...
			try {
				...
				System.out.println("현재 접속자 수 : "+ userMap.size()+"입니다.");
				while (in != null) {
					//받은 메시지를 모든 사용자에게 보냄
				...
				userMap.remove(name);//HashMap 에서 제거
